...
**Status:** Active — **M06** and **M07** **complete** on **`main`**; **Phase B — Runtime Seam Normalization** continues; **M08** (**bounded `AuroraImage` seam**) is implemented on branch **`m08-image-migration`** and **pending merge** to **`main`** (see Milestone ledger). **Phase A — Governance and Safety** is **closed**.  
**Last updated:** 2026-03-27 — **M07 closeout** (on **`main`**): merged **PR #8**; merge commit **`ca31fa7af428b1335af4b34f45bca639e0cec731`**; **merge time (UTC):** **2026-03-27T20:50:35Z**. **Last fully enumerated `main` tip before M08 work in this record:** **`002f6f8a9d1637b7f4f842beb6c4b23facb4c18c`** (M07 summary; aligns with pre-M08 baseline). **M08:** branch **`m08-image-migration`** — **primary implementation commit** **`90ea5722187993942408bcca15405a422875a8e6`**. **PR / merge / CI** identifiers are recorded in the **M08** ledger row when the milestone closes on **`main`**.
...
**Phase state:** **Phase A — Governance and Safety** is **closed**. **Phase B — Runtime Seam Normalization** is **authorized**; **M06** (**PR #7**) and **M07** (**PR #8**) are **closed** on **`main`**. **M08** (bounded first-party **`AuroraImage`** / **`image.py`** seam) is **in progress** on **`m08-image-migration`** — **pending merge**. Next after M08: **M09** (workspace `docs/milestones/M09/`; stub at closeout).
...
### Next milestone (M08 → M09)

**M08** (**bounded image seam on Dispatcher and loader seams**) introduces **`src/aurora/runtime/image.py`**: **`AuroraImage`** (`from_file`, `from_bytes`), **`ImageCreationError`**, injection of **`Dispatcher`** and **`LibraryLoader`**, no raw **`ctypes`** in **`image.py`**, fake-backed tests, verifier tracking — **not** upstream Tasks **`image.py`** edits, domain smoke tests, task bases, kernel work, or MediaPipe/native correctness claims (see `docs/runtime_seam_framing.md` §1b, §6b).

**M09** is next after M08 closes on **`main`** (workspace `docs/milestones/M09/`; seeded at M08 closeout).
...
- **Further Phase B seam work** (domain smoke tests; upstream Tasks **`image.py`** / fork wiring) — **M08** delivers the **first-party** bounded **`AuroraImage`** surface in **`src/aurora/runtime/image.py`** (not upstream migration); see `docs/runtime_seam_framing.md` and `docs/runtime_surface_strategy.md`
...
| **M08** | Bounded `image.py` migration onto Dispatcher and loader seams (first-party `AuroraImage`) | **Pending merge** | 2026-03-27 | Third **Phase B** slice: **`src/aurora/runtime/image.py`** — **`AuroraImage`**, **`ImageCreationError`**, route through **`Dispatcher.dispatch`** + **`LibraryLoader.shared_library`**, no **`ctypes`** in **`image.py`**, **`tests/test_image_runtime.py`**, verifier + **`docs/runtime_seam_framing.md`** + **`DEVELOPMENT.md`**; **not** upstream Tasks **`image.py`**, domain smoke, task bases, kernel, MediaPipe parity. | Branch **`m08-image-migration`**; **primary implementation commit** **`90ea5722187993942408bcca15405a422875a8e6`**. **Proves (intent):** first-party bounded image seam + dispatcher routing + deterministic failure in tests with fakes. **Does not prove:** decode correctness, native behavior, real host libraries. **Closeout** (merge commit, PR #, **`ci` / `repo-safety`**, date/time UTC): **record when merged to `main`**. |
...
- `docs/runtime_seam_framing.md` — **Phase B seam framing: `Dispatcher` / `LibraryLoader`, M07 `SharedLibraryLoader`, M08 bounded `AuroraImage`, LIVE_STREAM honesty** (M06–M08)
