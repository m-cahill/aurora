...
**Status:** Active — **Phase A — Governance and Safety** is **closed**; **Phase B — Runtime Seam Normalization** is **closed** at **M10** (governance closeout; tracked **Phase B** seam set satisfied by **M06**–**M09** on **`main`**; **upstream Tasks `image.py` / fork wiring** remains **deferred** and was **not** required for Phase B closure in this repository); **Phase C — Abstraction and Deduplication** is **authorized** — **M11** **complete** (first substantive **Phase C** milestone: measured **`src/aurora/`** line coverage + enforced floor in **`ci` / `repo-safety`**); **M12** **complete** (branch coverage **`branch = True`** + **separate** line and branch floors via **`scripts/check_coverage_thresholds.py`** and **`coverage.json`** — **`report:fail_under`** is **not** used in **`.coveragerc`** because coverage.py’s built-in fail-under is a **combined** line+branch ratio when both are measured, which would **not** preserve the M11 **statement**-only regression story); next substantive milestone: **M13** (workspace `docs/milestones/M13/`).  
**Last updated:** 2026-03-28 — **M12 on `main`:** **PR #13** merged — https://github.com/m-cahill/aurora/pull/13 — merge commit **`3bf18cf5acad9d8ca2df97cc4f3958cbe2a11900`**; **merge time (UTC):** **2026-03-28T05:57:13Z**; merge method **GitHub merge commit** (`gh pr merge 13 --merge --admin`). **PR head (pre-merge):** **`f2addfc3abf54e851801f17b2ddf3958efdc9a46`**. **`ci` / `repo-safety`** on PR head: **`23678773276`** (`push`), **`23678774673`** (`pull_request`) — **success**. Post-merge **`main`** merge push **`23678868568`** — **success** (head **`3bf18cf5acad9d8ca2df97cc4f3958cbe2a11900`**). **Primary implementation:** **`f2addfc3abf54e851801f17b2ddf3958efdc9a46`**; **`coverage==7.6.12`**, **`.coveragerc`** (`source=src/aurora`, **`branch=True`**); **`scripts/check_coverage_thresholds.py`**; **`Dispatcher` / `LibraryLoader`** Protocol stub lines **`# pragma: no cover`** (structural only). **M11** (PR #12) ledger row below records the **M11** **`.coveragerc`** **`fail_under=100`** line gate **at M11**; **M12** removes **`report:fail_under`** and enforces line + branch separately. **M10** record unchanged: **PR #11** merge **`7670b0b684727d36c692bee50aaa30fd672e56db`** (**2026-03-28T03:43:25Z**). **Post-closeout project record on `main`:** **`d05b383f25d0c4448555d8ff7089648a85e0b1f6`** — **`main`** **`23678888585`** (**success**). **Record alignment on `main`:** **`3ebdabdb74227b2e70611625434c5ef1a83fdd24`** — **`main`** **`23678896471`** (**success**). **Ledger enumeration on `main`:** **`828649ef1810057003e50fc255fd0734c6e4f186`** — **`main`** **`23678904630`** (**success**). **Last fully enumerated `main` tip in this record:** **`828649ef1810057003e50fc255fd0734c6e4f186`** (M12 closeout chain + CI **`23678904630`**). **Post-closeout record alignment:** **`afec182d6b502fe66034a09a4e040853aa4e5797`** — **`main`** **`23678916754`** (**success**; reconciles ledger wording to **828649** enumeration CI). **M11** on **`main`** (PR #12) narrative and closeout chain remain in the **ledger** (**M11** row) and prior milestone rows.
...
**M13** is next (workspace `docs/milestones/M13/`; scope locked at kickoff per *Execution Phase Boundaries*).
