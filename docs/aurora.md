...
| **M13** | Runtime surface coherence (internal error base + `AuroraImage` DRY) | **Open (PR #14 green; merge pending)** | 2026-03-28 | Third substantive **Phase C** slice: **`src/aurora/runtime/errors.py`** — **`AuroraRuntimeError`**; **`SharedLibraryLoadError`** / **`ImageCreationError`** subclass base (constructors/messages/chaining preserved); **`AuroraImage._from_dispatch`** (private); **`verify_repo_state.py`** requires **`errors.py`**; tests + **`DEVELOPMENT.md`**; lifecycle/resource abstraction **inspected, not extracted**; **no** task bases, **no** `mediapipe/`. | **PR #14** — https://github.com/m-cahill/aurora/pull/14 — **open**. **PR head:** **`fcd9b0c16566d8bb983b351617e9545bf287c93e`** (includes **`aurora.md`** draft + runtime **`7b83a54f335e34632a91caa3c7da3e321dd391a9`**). **`ci` / `repo-safety`:** **`23697481689`** (`push`), **`23697482335`** (`pull_request`) — **success**. **Proves (pending merge):** minimal internal error taxonomy + DRY image creation on bounded seam; **100%** line/**100%** branch on **`src/aurora/`** maintained. **Does not prove:** MediaPipe/native correctness; upstream parity; task bases. **Merge commit** / **post-merge `main` CI:** **TBD** at authorized merge closeout. |
