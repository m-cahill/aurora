...
**Status:** Active — **M06**, **M07**, and **M08** **complete** on **`main`**; **Phase A — Governance and Safety** is **closed**; **Phase B — Runtime Seam Normalization** is **authorized** (slices **M06**–**M08** delivered); next tracked milestone: **M09** (workspace `docs/milestones/M09/`).  
**Last updated:** 2026-03-27 — **M08 closeout:** merged **PR #9**; merge commit **`ba0110dd8f9b41e60213d026c50dc6a893c1c8f6`**; **merge time (UTC):** **2026-03-27T23:27:38Z**; merge method **GitHub merge commit** (`gh pr merge 9 --merge --admin`; default merge **blocked** by base branch policy). **PR head:** **`6c59f3ea7cdcae33981a10af0d327a5094d4ed01`**; **primary implementation commit:** **`90ea5722187993942408bcca15405a422875a8e6`**. **`ci` / `repo-safety`** green on PR head (**`23669305536`** `push`, **`23670441763`** `pull_request`), post-merge **`main`** merge push **`23671790446`** (head **`ba0110d`**), and closeout doc commit on **`main`** **`dc44fd72d98ca27ac9de6648f0ab83aa3094527f`** — **`main`** run **`23671809738`** (**success**). **Authoritative `main` tip after M08 closeout project record:** **`dc44fd72d98ca27ac9de6648f0ab83aa3094527f`**.
...
**Phase state:** **Phase A — Governance and Safety** is **closed**. **Phase B — Runtime Seam Normalization** is **authorized**; **M06** (**PR #7**), **M07** (**PR #8**), and **M08** (**PR #9**) are **closed** on **`main`**. Next: **M09** (workspace `docs/milestones/M09/`).
...
**M08** is **closed** on **`main`** (**PR #9**, merge **`ba0110dd8f9b41e60213d026c50dc6a893c1c8f6`**). It delivered the bounded first-party **`AuroraImage`** seam — see Milestone ledger and `docs/runtime_seam_framing.md` §1b.

**M09** is next (workspace `docs/milestones/M09/`; plan and toolcalls stubbed at M08 closeout). Scope is **not** fixed here; Phase B deferrals still include **domain smoke tests** and **upstream fork wiring** (see `docs/runtime_surface_strategy.md`).
...
- **Further Phase B seam work** (domain smoke tests; upstream Tasks **`image.py`** / fork wiring) — **M08** delivered the **first-party** bounded **`AuroraImage`** surface in **`src/aurora/runtime/image.py`** (not upstream migration); remaining items **deferred** to **M09+**; see `docs/runtime_seam_framing.md` and `docs/runtime_surface_strategy.md`
